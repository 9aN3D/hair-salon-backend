...
public class SettingInitializer {
    ...
    public void createIfNecessary() {
        var settings = settingFacade.findAll(new SettingGroupFindQuery(null));
        if (settings.isEmpty()) {
            getDefaultSettings().forEach((key, value) -> settingFacade.create(new SettingCreateCommand(key, value)));
        }
    }
    ...
    private Map<SettingIdDto, String> getDefaultSettings() {
        ...
        result.put(SALON_START_HOUR_WORK, "09:00");
        result.put(SALON_END_HOUR_WORK, "18:00");
