package pl.edu.wit.common.configuration.init;
...
public class HairdresserInitializer {
    ...
    public void createIfNecessary() {
        var hairdresserPage = hairdresserService.findAll(new HairdresserFindQuery(), new PageableParamsQuery(0, 1, PageSliceSort.empty()));
        if (!hairdresserPage.hasContent()) {
            ...
            prepareHairdresserCreateCommands()
                    .stream()
                    .map(hairdresserService::create)
                    .forEach(hairdresserId -> hairdresserService.uploadPhoto(hairdresserId, fileUploadCommandIterator.next()));
        }
    }
    ...
    private Map<String, String> collectProductNameToProductId() {
        return productService.findAll(new ProductFindQuery(), new PageableParamsQuery(0, 11, PageSliceSort.empty())).getContent()
                .stream()
                .collect(toMap(ProductDto::getName, ProductDto::getId));
    }
